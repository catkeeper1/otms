...
public class HibernateRestPaginationService {
	...
	public QueryResponse<Object> query(final String hql, final Map<String, Object> params, final Function<Object[], ?> mapper, final Long maxNoRecordsPerPage){
	...
	private String getHQLForTotalNoRecords(String queryStr){
		...
		do {
			if(i < 0){
				throw new SystemException("cannot find a top level 'FROM' from query string: '" + queryStr + "' . The i is < 0 already");
			}
			...
			if(fromIndex < 0){
				throw new SystemException("cannot find a top level 'FROM' from query string: '" + queryStr + "' . Cannot find 'FROM'. The i = " + i);
			}
